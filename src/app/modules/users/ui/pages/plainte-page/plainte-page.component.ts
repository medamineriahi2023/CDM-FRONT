...
@Component({
  selector: 'app-plainte-page',
  templateUrl: './plainte-page.component.html',
  styleUrls: ['./plainte-page.component.css']
})
export class PlaintePageComponent implements OnInit {
  ...
  open(content: any) {
    ...
    this.isEdit = false;
    this.modalTitle = this.locale == "ar" ? "إضافة شكوى جديدة" : "Ajouter une nouvelle Plainte";
  ...
  createForm(){
    this.formGroup = this.formBuilder.group({
      ...
      plainter: ['', [Validators.required]],
      adresseAccuser: ['', [Validators.required]],
      adressePlainter: ['', [Validators.required]],
      adrBien: ['', [Validators.required]],
      objetPlainte: ['', [Validators.required]],
  ...
  onSubmit() {
    ...
    plainte.numPlainte = this.formGroup.get("num")?.value;
    const date = this.formGroup.get("date")?.value;
    let d = new Date();
    d.setFullYear(date.year, date.month, date.day);
    plainte.date = d;
    plainte.accusateur = this.formGroup.get("accuser")?.value;
    plainte.plaignant = this.formGroup.get("plainter")?.value;
    plainte.adressePlaignant = this.formGroup.get("adressePlainter")?.value;
    plainte.adresseAccusateur = this.formGroup.get("adresseAccuser")?.value;
    plainte.adrBien = this.formGroup.get("adrBien")?.value;
    plainte.objetPlainte = this.formGroup.get("objetPlainte")?.value;
    plainte.remarques = this.formGroup.get("remarques")?.value;
    if (this.file){
      this.fileService.importFile(this.file!).subscribe(e => {
        const fileBlob = new FileBlob();
        // @ts-ignore
        fileBlob.id = e?.id;
        plainte.fileBlob= fileBlob;
        this.plainteService.create(plainte).subscribe(e => {
          this.plainteService.getAll().subscribe(e => this.plaintes = e.splice(0, this.select));
  ...
  async getAllData(): Promise<void> {
    try {
      const allPlainteData = await this.plainteService.getAll().toPromise();
      this.allPlaintesData = allPlainteData;
      this.plaintes = allPlainteData?.splice(0, this.select);
    } catch (error) {
      console.error(error);
    }
  }
  ...
  selectPaginator(v: any) {
    this.plainteService.getAll().subscribe(e => this.plaintes = e.splice(0, v));
  }
  ...
  onSwitchChange(val: any) {
    ...
    if (val.target.checked) {
      ...
      this.receiverService.updatePartial(service, this.fields).subscribe(e => {
        this.receiverService.getAll().subscribe(e => {
          this.plaintes = e.splice(0, this.select);

...
@Component({
  selector: 'app-tracking-motion-page',
  templateUrl: './tracking-motion-page.component.html',
  styleUrls: ['./tracking-motion-page.component.css']
})
export class TrackingMotionPageComponent implements OnInit {
  ...
  faFilePdf=faFilePdf
  data?: TrackingMotion[];
  trackingMotions?: TrackingMotion[];
  submitted = false;
  formGroup!: FormGroup;
  isEdit?: boolean;
  modalTitle?: string;
  allTrackingData?: TrackingMotion[];
  select = 5;
  isLoading = false;
  fields?: string;
  file?: File;
  ...
  open(content: any) {
    this.isEdit = false;
    this.modalTitle = this.locale == "ar" ? "الاحالة بالجملة" : "Affectation groupée";
  ...
  createForm(){
    this.formGroup = this.formBuilder.group({
      ...
      service: [new Date(), [Validators.required]],
    });
  }

  async getAllData(): Promise<void> {
    try {
      const allTrackingData = await this.trackingService.getAll().toPromise();
      this.allTrackingData = allTrackingData;
      this.trackingMotions = allTrackingData?.splice(0, this.select);
    } catch (error) {
      console.error(error);
    }
  }
  ...
  selectPaginator(v: any) {
    this.trackingService.getAll().subscribe(e => this.trackingMotions = e.splice(0, v));
  }
  ...
  addValue(event: Event) {
    ...
    this.formGroup.get('num')?.setValue(this.selectedNum?.join(","));
  }

  handleChecked(checkedValue:string){
    const t = checkedValue.split(",");
    if (!this.selectedItems?.includes(t[0])){
    this.selectedItems?.push(t[0]);
    this.selectedNum?.push(t[1]);
    }
  ...
  handleUnChecked(checkedValue:string){
    const t = checkedValue.split(",");
    if (this.selectedItems?.includes(t[0])){
      this.selectedItems?.splice(this.selectedItems.indexOf(t[0]), 1);
      this.selectedNum?.splice(this.selectedItems.indexOf(t[1]), 1);
    }
  }
}
